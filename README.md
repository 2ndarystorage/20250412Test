...
- 対話型CLIでプロバイダ（OpenAI / Claude / Gemini / LMstudio）を選択し、日本のことわざと意味を生成して表示する
- APIキーは環境変数（`.env`）から読み込み、LMstudioはOpenAI互換API経由で利用する
...
- Usable: 基本の生成とエラーメッセージはあるが、テスト/CIや配布設定がない

## Program Summary

- 対話型CLIでプロバイダ（OpenAI / Claude / Gemini / LMstudio）を選び、日本のことわざとその意味を1つ生成して表示する
- APIキーは環境変数（`.env`）から読み込み、LMstudioはOpenAI互換API経由で利用する（`LMSTUDIO_BASE_URL` / `LMSTUDIO_MODEL`）
...
- Usable: 単一スクリプトで基本機能は動くが、テスト/CIや配布設定がない

## Program Summary

- 対話型CLIでOpenAI/Claude/Gemini/LMstudioのいずれかを選び、日本のことわざと意味を生成して表示する
- APIキーは環境変数（`.env`）から読み込み、LMstudioはOpenAI互換API（`LMSTUDIO_BASE_URL`/`LMSTUDIO_MODEL`）で利用する
...
- Usable: 単一スクリプトで基本動作は揃っているが、テスト/CIや配布設定がない
